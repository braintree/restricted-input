...
IE9Strategy.prototype._keydownListener = function (event) {
  ...
  if (this._isDeletion(event)) {
    ...
    // IE9 does not update the input.value during the key event
    // pull off event.key and add it to input value before
    // formatting
    ...
    if (selection.start + 1 === newValue.length) {
...
IE9Strategy.prototype._reformatAfterPaste = function () {
  ...
  setTimeout(function () {
    setSelection(input, selection.start + 1, selection.end + 1);
  }, 0);
};
